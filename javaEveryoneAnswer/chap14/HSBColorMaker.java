...
Rectangle rect;
	...
	//スライダとテキストフィールドの設定メソッド (スライダ，テキストフィールド，スライダのメモリ間隔)
	void setSliderAndTextField(Slider slider, TextField tf, double tickUnit){
		...
		slider.valueProperty().addListener((ov, oldValue, newValue) -> {
			tf.setText(String.format("%.2f", newValue.doubleValue()));
			mixColor();
		});
		tf.setOnAction((event) -> {
			double v;
			try{ //データを取得
				v = Double.parseDouble(tf.getText()); //double型に変換
			} catch (NumberFormatException error){ v = 0.0; } 
			if(v > 0){
				slider.setValue(v);
				mixColor();
			}
	...
	void mixColor(){
		rect.setFill(Color.hsb(hueS.getValue(), saturationS.getValue()/100, brightS.getValue()/100));
	}
